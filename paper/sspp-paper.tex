...
\bottomrule
\end{tabular}
\caption{VORTEX vs Naive RAG vs Router (Contains, 50 QA). Router applies to \texttt{qwen2.5:7b} only.}
\label{tab:main}
\end{table}
...
Future work will explore cross-model routing (e.g., fast path with \texttt{llama3.1:8b} at 72\% baseline, slow path with \texttt{qwen2.5:7b} VORTEX) to combine the strengths of different LLMs, integration of semantic search via FAISS, and scaling evaluation to HotpotQA~\cite{yang2018hotpotqa} and other established benchmarks.
