...
\bottomrule
\end{tabular}
\caption{VORTEX vs Naive RAG vs Router (Contains, 50 QA). Same-model Router uses \texttt{qwen2.5:7b}; $^\dag$Cross-model Router uses \texttt{llama3.1:8b} (fast) and \texttt{qwen2.5:7b} VORTEX (slow).}
\label{tab:main}
\end{table}
...
The same-model router uses one LLM for both paths. However, \texttt{llama3.1:8b} has the strongest direct answer (72\% baseline) while \texttt{qwen2.5:7b} excels at VORTEX spirals (74\%), suggesting a cross-model architecture: fast path with \texttt{llama3.1:8b} (stronger System 1) and slow path with \texttt{qwen2.5:7b} VORTEX (stronger System 2).

We evaluate this on the same 50-question benchmark. The fast path handles 41/50 questions (82\%) at 93\% accuracy; the slow path recovers 4 of 9 fallbacks (44\%). Overall: \textbf{84\%} (42/50)---a $+12\%$ gain over the fast-model baseline and $+2\%$ over the same-model router. Total time: 9.8 min (11.8 s avg). This is the best result on our benchmark.
...
Future work will explore additional model combinations (e.g., larger LLMs, open-weight models), integration of semantic search via FAISS, and scaling evaluation to HotpotQA~\cite{yang2018hotpotqa} and other established benchmarks.
